...
class AddressCreateView(APIView):
    ...
    def post(self, request):
        try:
            ...
            # Get all address records within a transaction
            with transaction.atomic(using="fred"):
                address = serializer.save()

                # Log successful creation
                logger.info(f"Payment created successfully: ID {address.id}")
...
class AddressOneView(APIView):
    ...
    def get(self, request, address_id):
        ...
        try:
            ...
            address_serializer = reference.AddressModelSerializer(address)
            return Response(
                {
                    "message": "Successfully retreived address",
                    "value": address_serializer.data,
                },
                status=status.HTTP_200_OK,
            )
